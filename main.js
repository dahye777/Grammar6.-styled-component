...
// test("test"); //test
// test`test`; //['test']
// //에크마6문법에서는 ``으로 인해 함수의 호출과 인수값을 ``으로 처리하면
// // 함수의 값이 배열로 반환됩니다 => 이것을 이용해서 DOM요소를 만들때 동시에 스타일도 입히는
...
두번째()안에 인수를 넣으면 반환된 함수에 인수를 대입해서 최종 호출하겠다는 의미가됩니다
()는 안에 매개변수자리에 인수를 넣는 공간입니다
리턴하는 함수 안에 인수를 넣으려면 ()을 한번 더 옆에 써주면 됩니다
...
//만약 h1의 폰트사이즈, 폰트 패밀리, 라인하이트, 컬러등등 한번에 적용시켜 호출한다면
//위 코드는 한계가 존재합니다
